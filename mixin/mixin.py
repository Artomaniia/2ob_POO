...
# Состояние гонки или Race Condition
...
# установим это время, чтобы точно зафиксировать увеличение счетчика и другие потоки "видели"
# именно обновленное значение
RACE_CONDITION_SAFETIME = .0001
...
if __name__ == "__main__":
    ...
    def worker(updates):
        for _ in range(updates):
            counter.increment()
            time.sleep(RACE_CONDITION_SAFETIME)  # маленькая задержка для увеличения шанса race condition
